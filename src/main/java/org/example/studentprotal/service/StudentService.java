...
public class StudentService {
    ...
    @Transactional
    public String forgotPassword(String fullName, String newPassword) {
        // Assuming studentDao.findByName(fullName) returns an Optional<Student>
        Student student = studentDao.findByName(fullName)
                .orElseThrow(() -> new EntityNotFoundException("Student not found with full name: " + fullName));
        // Optionally encode the password (if using an encoder)
        // e.g., student.setPassword(passwordEncoder.encode(newPassword));
